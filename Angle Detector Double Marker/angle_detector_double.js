AFRAME.registerComponent('mt-angle-detector-double', {
  schema: {
  ...
  init: function () {
    console.log("Initializing mt-angle detector component double");
  },
    ...
    for (let i = 0; i <= 360 ; i += this.data.threshold) {
      ...
      if (i > this.angle) {
        ...
        if ( level != this.lastAngle) {
          ...
          const event_rotation_double = new CustomEvent('event_rotation_double', {
            detail: {
              time: time,
              object1: this.el,
